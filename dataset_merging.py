...
from process_functions import get_ids_and_files_within_given_range
from shutil import copyfile
from process_functions import percentile_remove_outlier
import os
import re
import sys
import numpy as np
...
class mergeSimilarDataset():
    ...
    def qq_plot(self, df_samp, df_clu):
        ...
        :return: png file of Q-Q plot of these 2 enterprises and slope, intercept and total fit error of fitted regression line
        """
        ...
        print "%s: Start drawing Q-Q plot using data from sample %s and cluster %s." %(str(dt.datetime.now()), str(samp_id), str(ref_id))
        ...
        # calculatet the error from real percentiles to predicted percentiles: as same as mean squared error
        ...
        outfile = "%s/enterprise-%s-VS-cluster-%s.qqplot.png" %(outdir, str(samp_id), str(ref_id))
        plt.savefig(outfile)
        print "%s: Plotting Q-Q plot done! The plot is stored at %s." %(str(dt.datetime.now()), outfile)
        plt.close()
        return p[0], p[1], pct_error

    def merge_similar_dataset(self, qq_slope_range=(0.85, 1.15), qq_intercept=10, override=True):
        """
        Merge data set within similar range used for NN training
        ...
        ids, train_files, itv_files = get_ids_and_files_within_given_range(self.input_dir, self.enter_id_range, interval=True)
        # search output directory if cluster files exist. If not, copy first file in files list to output directory as cluster 1 file
        ...
        for samp_index, samp_file in enumerate(itv_files):
            ...
            df_samp_fil = percentile_remove_outlier(df_samp, 0)
            ...
            # Merge sample train data to most similar cluster (slope in qq plot closest to 1).
            ...
            for ref_index, ref_file in enumerate(cluster_interval_files):
                ...
                df_clu_fil = percentile_remove_outlier(df_clu, 0)
                ...
                print "KS test results: ks-statistic: %f; p_value: %f" % (ks_sta, p_value)
                ...
                if qq_slope >= qq_slope_range[0] and qq_slope <= qq_slope_range[1]:
                    # if p_value > ks_pv:
            ...
            if not slope_to_1:
                ...
                print "%s: Data distribution of sample %s is apparently distinct from existed clusters. " \
                      "Assign it as a new cluster %s." %(str(dt.datetime.now()), str(samp_id), str(new_cluster_id))
                interval_src_file = "%s/%s.intervals.csv" %(self.input_dir, str(samp_id))
                interval_dst_file = "%s/%s-%s.intervals.csv" %(interval_dir, self.outfile_prefix, str(new_cluster_id))
                train_src_file = "%s/%s.csv" %(self.input_dir, str(samp_id))
                train_dst_file = "%s/%s-%s.csv" %(train_dir, self.outfile_prefix, str(new_cluster_id))
                copyfile(interval_src_file, interval_dst_file)
                copyfile(train_src_file, train_dst_file)
                # store cluster info
                cluster[str(new_cluster_id)] = np.array([int(samp_id)])
            else:
                # get the most similar cluster id
                cluster_id = min(slope_to_1, key=slope_to_1.get)    # type str
                print "%s: Data distribution of sample %s is most similar to cluster %s. " \
                      "Merge sample data with cluster data." %(str(dt.datetime.now()), str(samp_id), str(cluster_id))
                interval_dst_file = "%s/%s-%s.intervals.csv" %(interval_dir, self.outfile_prefix, cluster_id)
                train_dst_file = "%s/%s-%s.csv" %(train_dir, self.outfile_prefix, cluster_id)
                ...
                if cluster_id in cluster.keys():
                    cluster[cluster_id] = np.append(cluster[cluster_id], int(samp_id))
                else:
                    cluster[cluster_id] = np.array([int(samp_id)])
        # output cluster info
        print "Cluster Info: %s" %str(cluster)
        ...
        print "%s: Output each cluster consists (enterprise ids) to file: %s." %(str(dt.datetime.now()), cluster_info_file)

    @staticmethod
    def merge_selected_dataset(clustering_ref_file, indir=".", infile_format="%s.csv", outdir=".", outfile_prefix="merge"):
        """
        :param indir=".": input directory for training set files
        :param infile_format="%s.csv": read in file in such format
        ...
        for cluster_index, cluster_name in enumerate(cluster_list):
            ...
            for enter_id in df_ref.ix[:, cluster_index].dropna():  ## drop na
                enter_file = input_dir + "/" + infile_format %(str(int(enter_id)))
            ...
            outfile = outdir + "/" + outfile_prefix + "-%s.csv" %(str(cluster_name))
            df_train.to_csv(outfile)


if __name__ == '__main__':
    cluster_file = "reference-file-for-clustering"
    # check if cluster file exists
    if not os.path.exists(cluster_file):
        output_dir = "path-to-your-output-directory"
        input_dir = "path-to-your-input-directory" # generated by trainingset selection script
        enter_id_file = input_dir + "/filtered_enterprise_id_list.csv"
        enter_id_range = (1001, 6000)
        outfile_prefix = 'cluster'
        override=False
        # ks_pv = 0.1
        ...
        print "Message during clustering enterprise data could be found in %s" %log_file
        sys.stdout = log_file_handler
        # check if input data file exists
        if not os.listdir(input_dir):
            raise ValueError("Input files are not found in %s. Check input directory," %input_dir)
        # similarity assessment index
        print "%s: Start clustering input enterprise dataset from %s to %s..." %(str(dt.datetime.now()), str(enter_id_range[0]), str(enter_id_range[1]))
        obj_merge.merge_similar_dataset(qq_slope_range=qq_slope_range, qq_intercept=qq_intercept, override=override)
        print "%s: Clustering done! You could find them in %s" %(str(dt.datetime.now()), output_dir)
        log_file_handler.close()
        sys.stdout = stdout_backup
    else:
        input_dir = "path-to-your-input-directory"
        output_dir = input_dir + "/cluster_merged"
        mergeSimilarDataset.merge_selected_dataset(cluster_file, indir=input_dir, outdir=output_dir)
