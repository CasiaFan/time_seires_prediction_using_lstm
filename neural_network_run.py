...
class NeuralNetwork():
    def __init__(self,
        ...
        :param **kwargs: lstm_output_dim=4: output dimension of LSTM layer; activation_lstm='tanh': activation function for LSTM layers; activation_dense='relu': activation function for Dense layer; activation_last='sigmoid': activation function for last layer; drop_out=0.2: fraction of input units to drop; np_epoch=10, the number of epoches to train the model. epoch is one forward pass and one backward pass of all the training examples; batch_size=32: number of samples per gradient update. The higher the batch size, the more memory space you'll need; loss='mean_square_error': loss function; optimizer='rmsprop'
        """
    ...
    def NN_prediction(dataset, model_save_path):
        ...
        predict_dummy_classes = model.predct_classes(dataset)
        class_prob = model.predict_proba(dataset)
        # restore dummy labels to initial labels
        predict_class = np.asarray([list(x).index(1) + 1 for x in dataset])
    ...
    def model_train_predict_test(self, input_file_regx="^(\d+)\.csv", override=False):
        ...
        for id_index, id_file in enumerate(files):
            ...
            # bin date y
            df_bin, bin_boundary = binning_date_y(df_scale, y_col=2+self.training_set_length, n_group=5)
            # get train and test dataset
            print "Randomly selecting training set and test set..."
            all_data_x = np.asarray(df_bin.ix[:, 1:1:self.training_set_length])
            all_data_y = np.asarray(df_bin.ix[:, 1+self.training_set_length])
            # format train, test, validation data
            sub_train, sub_test, val_train, val_test = train_test_split(all_data_x, all_data_y, test_size=self.test_size)
            ...
            match_count = (np.asarray(val_predict_class) == np.asarray(val_test)).sum()
            print "Precision using validation dataset is {}".format(match_count / total_sample_count)


def main():
    output_dir = "C:/Users/fanzo/Desktop/RNN_prediction/enterprises-train.5-5/cluster_merged"
    ...
    obj_NN.model_train_predict_test(override=False)
